  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16 text-[15px]">
      ...
      <p className="mt-3 leading-7 text-gray-800">
        ...
        한 가지 예외로, 인공지능(AI)이 영상 프레임을 판독하거나 정지컷을 만들어 주는 릴스
        템플릿(이하 &ldquo;AI 템플릿&rdquo;)을 이용자가 골라 쓸 때는 이용자가 고른 영상
        프레임 1장이 운영팀 서버를 거쳐 국외 AI 업체(구글·OpenAI)로 전송되고, 서버는 결과를
        돌려준 즉시 그 프레임을 지웁니다(제5조). 성별·연령대는 선택 항목이라 답하지 않아도
        됩니다(제1조). 서비스 개선을 위한 앱 이용 기록은 외부 분석 도구로, 광고 성과 측정을
        위한 설치·실행 기록은 외부 측정 도구로 전송됩니다(제5조). 광고 식별자(IDFA)는
        이용자가 앱 추적을 허용한 경우에만 씁니다(제10조).
      </p>
      ...
      <div className="mt-8 rounded-lg border border-gray-300 bg-gray-50 p-5">
        ...
        <ul className="mt-3 list-disc space-y-1 pl-5">
          <li><strong>계정 정보:</strong> 소셜 로그인으로 가입할 때 로그인 제공자·제공자가 발급한 사용자 식별자·이메일을 수집(제1조). 닉네임은 서비스가 만들어 배정하며 수집하지 않습니다. 성별·연령대는 선택 항목으로, 건너뛰어도 모든 기능을 쓸 수 있습니다(제1조)</li>
          <li><strong>처리 항목:</strong> 러닝 기록(HealthKit·기록 화면 캡처) — 기기에서 처리하고 측정값을 서버에 백업(GPS 경로 제외), 러닝 장소 지명 표시를 위해 경로 첫 좌표 1개를 Apple에 전송(제5조·제11조) / 사진·영상 — 이용자 기기 안에서만 처리·보관. AI 템플릿을 쓸 때만 이용자가 고른 영상 프레임 1장을 운영팀 서버를 거쳐 국외 AI 업체(구글·OpenAI)로 전송(제5조) / 기기 정보·앱 이용 기록 — 서버 등록·분석 도구로 전송(제5조) / 광고 유입 정보(광고 식별자는 추적 허용 시에만) — 광고 성과 측정 도구로 전송(제5조)</li>
          <li><strong>처리 목적:</strong> 베스트컷 추천, 기록 오버레이·릴스 생성, AI 템플릿의 자세 판독과 정지컷 생성, 계정 로그인과 기록 백업·복원, 성별·연령대별 러닝 통계와 맞춤 추천, 서비스 품질 개선, 광고 성과 측정(어떤 광고를 보고 설치했는지 확인)</li>
          <li><strong>보유 기간:</strong> 계정 정보(성별·연령대 포함)와 서버 백업 기록은 계정 삭제를 요청하면 30일 유예 뒤 파기(제3조), AI 업체로 보낸 영상 프레임은 결과를 받는 즉시 서버에서 삭제(제3조), 기기 내 정보는 앱 삭제 시 파기(익명 기기 식별자만 예외 — 기기에 남습니다, 제3조), 앱 이용 기록은 위탁 업체 보관 후 목적 달성 시 파기</li>
          <li><strong>제3자 제공:</strong> 원칙적으로 하지 않음. 광고를 집행하는 동안 설치·공유 완료 이벤트를 Meta에 제공할 수 있음(제4조, 현재는 제공하지 않음). 앱 이용 기록 분석·광고 성과 측정·좌표의 지명 변환·AI 템플릿의 프레임 판독과 정지컷 생성 업무는 국외 업체에 위탁(제5조)</li>
      ...
      <Section title="제1조 (처리하는 개인정보의 항목 및 수집 방법)">
        <p>
          ...
          이용자의 기기 안에서만 처리하고(AI 템플릿을 쓸 때 보내는 프레임 1장만 예외), 러닝
          기록의 측정값은 백업을 위해 서버에 저장합니다.
        </p>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left">
            ...
            <tbody>
              ...
              <tr className="border-b border-gray-200">
                <td className="p-2 font-semibold">프로필 정보(선택)</td>
                <td className="p-2">성별(여성·남성), 연령대. <strong>둘 다 선택 항목이며 답하지 않아도 모든 기능을 똑같이 쓸 수 있습니다.</strong> 출생연도를 입력받지만 서버에는 10세 단위 연령대(예: 20대)로 바꿔 저장하고 출생연도 자체는 남기지 않습니다. 이름·생일(월일)·전화번호는 받지 않습니다</td>
                <td className="p-2">로그인 뒤 한 번 나오는 폼(건너뛸 수 있음)과 내 정보 화면에서 이용자가 직접 입력. 네이버 로그인은 이용자가 네이버 동의 화면에서 선택 동의한 경우에만 네이버로부터 받음</td>
              </tr>
              <tr className="border-b border-gray-200">
              ...
              <tr className="border-b border-gray-200">
                ...
                <td className="p-2">러닝 시간대에 촬영된 사진·영상(베스트컷 추천·편집 대상). 기기 안에서만 처리하며, <strong>AI 템플릿을 쓸 때만</strong> 이용자가 영상에서 고른 프레임 1장(가로 512픽셀로 줄인 정지 이미지 — 얼굴이 나올 수 있음)을 운영팀 서버를 거쳐 제5조의 AI 업체로 보냅니다. 서버에는 어떤 템플릿인지도 함께 보내지만 AI 업체로는 프레임만 갑니다. 영상 전체나 다른 프레임은 보내지 않습니다</td>
                <td className="p-2">이용자가 허용한 경우 사진 라이브러리에서 읽기. 프레임은 이용자가 AI 템플릿에서 영상과 멈출 지점을 직접 고를 때만 만들어짐</td>
              </tr>
              <tr className="border-b border-gray-200">
        ...
        <p>
          ...
          <strong>성별·연령대도 선택입니다.</strong> 폼에서 건너뛰면 다시 묻지 않으며, 답한 값은
          내 정보 화면에서 언제든 바꿀 수 있습니다.
        </p>
      </Section>
      ...
      <Section title="제2조 (개인정보의 처리 목적)">
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>베스트컷 추천(사진·영상 품질과 얼굴·구도를 기기 내에서 평가). <strong>얼굴은 인식·평가만 하며, 아래 AI 템플릿의 정지컷 생성을 빼면 생성·변형·보정하지 않습니다.</strong></li>
          <li>기록 오버레이·릴스·프레임 콘텐츠 생성과 편집</li>
          <li>AI 템플릿의 자세 판독과 정지컷 생성. 이용자가 고른 영상 프레임 1장에서 손 위치와 기록 글자를 얹을 자리를 판독하고, 템플릿에 따라 그 프레임을 바탕으로 효과를 더한 정지컷 이미지를 새로 만듭니다. <strong>정지컷을 만들 때는 프레임 속 인물의 얼굴·몸이 AI에 의해 다시 그려질 수 있습니다.</strong> 만들어진 정지컷은 기록 글자를 얹을 자리 판독과 구도 검수를 위해 구글을 한 번 더 거친 뒤 이용자 기기에만 저장되며, 누구인지 알아보는 처리(같은 사람 찾기, 얼굴 특징 추출)는 하지 않습니다</li>
          <li>계정 생성과 로그인 상태 유지, 기기를 바꾸거나 앱을 다시 설치했을 때의 기록 백업·복원</li>
          <li>성별·연령대별 러닝 통계와 맞춤 추천(선택 항목에 답한 이용자만 해당)</li>
      ...
      <Section title="제3조 (개인정보의 처리 및 보유 기간)">
        ...
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li>성별·연령대는 계정 정보와 함께 보관하고 계정 삭제 때 같이 파기합니다. 내 정보 화면에서 값을 바꿀 수 있으며, 답한 값을 아예 지우고 싶으면 운영팀(admin@rungle.app)에 문의해 주세요.</li>
          ...
          <li>
            <strong>AI 템플릿을 쓸 때 서버로 보낸 영상 프레임은 AI 업체의 결과를 앱에 돌려주는
            즉시 서버에서 삭제하며,</strong> 캐시·로그·저장소 어디에도 남기지 않습니다. 서버
            기록에는 작업 번호·요청한 계정·요청 시각·파일 크기·처리 상태만 남고, 프레임과
            결과물은 남지 않습니다. AI가 만든 정지컷은 이용자 기기에만 저장되고, 그 정지컷을
            쓴 초안을 지우면 함께 삭제됩니다.
          </li>
      ...
      <Section title="제5조 (개인정보 처리의 위탁 및 국외 이전)">
        ...
        <p>
          ...
          광고 성과 측정 업무, 러닝 장소 지명 표시를 위한 좌표의 지명 변환 업무, AI 템플릿의
          프레임 판독과 정지컷 생성 업무를 다음 업체에 위탁하고 있습니다. 이 업체들의 서버가
          국외에 있어 아래 항목이 국외로 이전됩니다.
        </p>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left">
            <thead>
            ...
            <tbody>
              ...
              <tr className="border-b border-gray-200">
                ...
                <td className="p-2">미국 / 이용자가 AI 템플릿에서 영상과 멈출 지점을 고르고 만들기를 실행할 때 / 운영팀 서버를 거쳐 네트워크(HTTPS 암호화) 전송</td>
                <td className="p-2">이용자가 고른 영상 프레임 1장(가로 512픽셀 정지 이미지 — 얼굴이 나올 수 있음), 그 프레임으로 AI가 만든 정지컷(기록 글자를 얹을 자리 판독과 구도 검수용)</td>
                <td className="p-2">프레임 속 손 위치 판독, AI가 만든 정지컷에서 기록 글자를 얹을 자리 판독과 구도 검수. OpenAI의 정지컷 생성이 안전 정책에 막히면 대신 정지컷 생성 / 판독 결과를 돌려줄 때까지. 구글은 유료 API 약관에 따라 프레임을 학습이나 제품 개선에 쓰지 않고, 금지 행위 감시 목적으로만 제한된 기간 보관합니다</td>
              </tr>
              <tr className="border-b border-gray-200">
                <td className="p-2 font-semibold">
                  OpenAI, L.L.C.
                ...
                <td className="p-2">미국 / 정지컷을 만드는 AI 템플릿에서 이용자가 만들기를 실행할 때 / 운영팀 서버를 거쳐 네트워크(HTTPS 암호화) 전송</td>
                <td className="p-2">이용자가 고른 영상 프레임 1장(가로 512픽셀 정지 이미지 — 얼굴이 나올 수 있음)</td>
                <td className="p-2">프레임을 바탕으로 효과를 더한 정지컷 이미지 생성 / 생성 결과를 돌려줄 때까지. OpenAI는 API로 받은 데이터를 학습에 쓰지 않으며, 남용 감시 목적으로 최대 30일 보관한 뒤 삭제합니다</td>
              </tr>
            </tbody>
          </table>
        </div>
        <p>
          ...
          원본 수치가 들어가지 않습니다.</strong> 예외는 두 가지입니다. 지명 변환을 위해 Apple로
          보내는 경로 첫 좌표 1개와, AI 템플릿을 쓸 때 구글·OpenAI로 보내는 영상 프레임
          1장(그리고 그 프레임으로 AI가 만든 정지컷)입니다. 프레임은 기록 오버레이를 입히기 전의 원본 화면이라 러닝 기록 수치가 담기지
          않으며, 프레임과 함께 계정 정보·기기 식별자·러닝 기록을 보내지 않습니다.
          국외 이전을 원하지 않으면 앱을 삭제하면 되고, 삭제한 뒤로는 새 기록이 전송되지
          않습니다. 광고 식별자만 막으려면 iOS 설정 &gt; 개인정보 보호 및 보안 &gt; 추적에서 앱
          추적을 끄면 됩니다(제7조).
        </p>
        <p>
          <strong>AI 업체로의 이전만 원하지 않으면 AI 템플릿을 쓰지 않으면 됩니다.</strong> 이
          템플릿에서 이용자가 직접 영상과 멈출 지점을 고르고 만들기를 실행하기 전에는 어떤
          프레임도 전송되지 않으며, 쓰지 않아도 그 템플릿 말고는 아무 기능도 제한되지 않습니다.
          구글·OpenAI는 운영팀의 일을 대신하는 수탁자로서 유료 API 약관에 따라 프레임을 자체
          목적이나 학습에 쓰지 못합니다. 위 두 업체 외의 AI 서비스로는 사진·영상을 보내지
          않으며, 이용 범위가 바뀌면 본 방침을 통해 공개합니다.
        </p>
      </Section>
      ...
      <Section title="제7조 (정보주체와 법정대리인의 권리·의무 및 행사 방법)">
        ...
        <ul className="list-disc space-y-1 pl-5">
          ...
          <li><strong>AI 업체로의 프레임 전송 거부:</strong> AI 템플릿을 고르지 않으면 됩니다. 다른 템플릿과 기능은 그대로 쓸 수 있습니다(제5조)</li>
          <li><strong>성별·연령대 변경:</strong> 앱의 내 정보 화면에서 바꿀 수 있습니다. 답한 값을 지우려면 운영팀에 문의해 주세요(제3조)</li>
      ...
      <Section title="제9조 (개인정보의 안전성 확보 조치)">
        ...
        <ul className="list-disc space-y-1 pl-5">
          <li><strong>기기 내 처리:</strong> 사진 분석·얼굴 평가·문자인식을 포함한 개인정보 처리를 이용자 기기 안에서 수행하고, iOS 앱 샌드박스로 다른 앱의 접근이 차단됩니다. AI 템플릿의 영상 프레임 1장만 예외로 서버를 거쳐 AI 업체로 가며, 서버는 결과를 돌려준 즉시 프레임을 지우고 저장하지 않습니다.</li>
          <li><strong>최소 저장:</strong> 출생연도는 10세 단위 연령대로만 저장하고 원본을 남기지 않습니다.</li>
      ...
      <Section title="제11조 (건강 데이터의 취급)">
        <p>
          ...
          파생값도 보내지 않습니다. 제5조의 AI 업체로 보내는 영상 프레임은 기록 오버레이를
          입히기 전의 원본 화면이라 건강 데이터가 담기지 않습니다.
        </p>
        <p>
      ...
      <Section title="제13조 (개인정보 처리방침의 변경)">
        <p>
          ...
          특히 위치정보(GPS 경로)의 서버 저장, 서버 푸시 알림(운영팀 서버에서 보내는 알림 —
          현재 제공 중인 러닝 감지 알림은 기기 안에서만 만들어지는 로컬 알림입니다), 영상
          전체나 여러 프레임을 외부 AI로 보내는 기능, 사진·영상을 서버에 저장하는 기능이
          도입되는 시점에 본 방침이 개정됩니다.
        </p>
        <p className="text-gray-600">
          이번 개정으로 선택 항목인 성별·연령대와 AI 템플릿의 국외 AI 이용이 반영되었습니다.
          개정 전 방침(2026년 9월 8일 시행)에서 달라진 부분은 프로필 정보(성별·연령대) 항목과
          AI 템플릿의 영상 프레임 전송(제1조), 자세 판독·정지컷 생성 목적과 성별·연령대별
          통계·맞춤 추천 목적(제2조), 성별·연령대의 보유와 프레임의 즉시 삭제(제3조),
          구글(Gemini API)·OpenAI로의 국외 이전과 거부 방법(제5조), 프레임 전송 거부와
          성별·연령대 변경 방법(제7조), 기기 내 처리의 예외와 연령대 최소 저장(제9조), AI로
          보내는 프레임에 건강 데이터가 없다는 설명(제11조)입니다.
        </p>
        <p className="text-gray-600">
          이번 개정은 공고와 동시에 시행합니다. 새로 추가된 처리(성별·연령대 수집, AI 템플릿의
          프레임 전송)는 이용자가 직접 고르는 기능에서만 일어나고, 이 방침을 공개한 뒤 배포되는
          앱 버전(1.2.0)을 설치한 때부터 시작되며, 그 전 버전에서는 일어나지 않습니다.
        </p>
        <p><strong>공고일 및 시행일: 2026년 9월 9일</strong></p>
      </Section>
    </main>
  );
}
