  ...
  return (
    <main className="mx-auto max-w-3xl px-6 py-16 text-[15px]">
      ...
      <p className="mt-3 leading-7 text-gray-800">
        ...
        서비스 개선을 위한 앱 이용 기록은 외부 분석 도구로 전송됩니다(제5조).
      </p>
      ...
      <div className="mt-8 rounded-lg border border-gray-300 bg-gray-50 p-5">
        ...
        <ul className="mt-3 list-disc space-y-1 pl-5">
          ...
          <li><strong>처리 항목:</strong> 러닝 기록(HealthKit·기록 화면 캡처) — 기기에서 처리하고 측정값을 서버에 백업(GPS 경로 제외) / 사진·영상 — 이용자 기기 안에서만 처리·보관 (일부 콘텐츠 분석 시 외부 AI 이용 가능, 제5조) / 기기 정보·앱 이용 기록 — 서버 등록·분석 도구로 전송(제5조)</li>
          <li><strong>처리 목적:</strong> 베스트컷 추천, 기록 오버레이·릴스 생성, 계정 로그인과 기록 백업·복원, 서비스 품질 개선</li>
          ...
          <table className="w-full border-collapse text-left">
            ...
            <tbody>
              ...
              <tr className="border-b border-gray-200">
                ...
                <td className="p-2">거리·시간·페이스·칼로리·심박수·케이던스·GPS 경로 등 러닝 워크아웃 데이터. 이 가운데 측정값(거리·시간·페이스·칼로리·심박수·케이던스·구간 기록)은 백업·복원을 위해 운영팀 서버에 저장됩니다. <strong>GPS 경로는 서버로 보내지 않으며, 서버는 GPS 경로를 받더라도 거절합니다.</strong></td>
      ...
      <Section title="제5조 (개인정보 처리의 위탁 및 국외 이전)">
        ...
        <p>
          이와 별개로, 운영팀은 서비스 품질 개선과 오류 대응을 위해 앱 이용 기록 처리 업무를
          다음 업체에 위탁하고 있습니다. 두 업체의 서버가 국외에 있어 아래 항목이 국외로
          이전됩니다.
        </p>
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left">
      ...
      <Section title="제11조 (건강 데이터의 취급)">
        <p>
          ...
          운영팀 서버(국내)에 저장되며, <strong>GPS 경로는 서버로 전송하지 않고 서버도 이를
          받지 않도록 만들어져 있습니다.</strong> Apple 정책에 따라 <strong>건강 데이터를
      ...
      <Section title="제13조 (개인정보 처리방침의 변경)">
        ...
        <p className="text-gray-600">
          이번 개정으로 소셜 로그인과 러닝 기록의 서버 백업이 반영되었습니다. 개정 전
          방침(2026년 8월 4일 시행)에서 달라진 부분은 계정 정보·기기 정보 수집 항목 신설(제1조),
          서버 보관과 계정 삭제 절차(제3조·제6조·제7조), 서버 운영 위탁(제5조), 러닝 기록
          측정값의 서버 저장(제11조)입니다.
        </p>
        <p className="text-gray-600">
          이번 개정은 공고와 동시에 시행합니다. 새로 수집하는 계정 정보는 이용자가 소셜
          로그인을 직접 선택할 때만 받고, 로그인하지 않고 이용하던 분들의 처리 내용은 이전과
          같아서 미리 알릴 기간을 두지 않았습니다. 개정 내용에 동의하지 않으시면 로그인하지
          않고 게스트로 이용하시거나, 이미 만든 계정은 앱의 내 정보 화면에서 삭제하실 수
          있습니다.
        </p>
        <p><strong>공고일 및 시행일: 2026년 8월 28일</strong></p>
      </Section>
    </main>
  );
}
